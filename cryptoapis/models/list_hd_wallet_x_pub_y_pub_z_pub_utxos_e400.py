...
LISTHDWALLETXPUBYPUBZPUBUTXOSE400_ONE_OF_SCHEMAS = ["InvalidPagination", "InvalidXpub", "LimitGreaterThanAllowed", "UriNotFound", "XpubNotSynced"]

class ListHDWalletXPubYPubZPubUTXOsE400(BaseModel):
    """
    ...
    oneof_schema_4_validator: Optional[XpubNotSynced] = None
    # data type: InvalidXpub
    oneof_schema_5_validator: Optional[InvalidXpub] = None
    actual_instance: Any
    one_of_schemas: List[str] = Field(LISTHDWALLETXPUBYPUBZPUBUTXOSE400_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ListHDWalletXPubYPubZPubUTXOsE400.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ListHDWalletXPubYPubZPubUTXOsE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound, XpubNotSynced. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ListHDWalletXPubYPubZPubUTXOsE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound, XpubNotSynced. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ListHDWalletXPubYPubZPubUTXOsE400:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ListHDWalletXPubYPubZPubUTXOsE400:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ListHDWalletXPubYPubZPubUTXOsE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound, XpubNotSynced. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ListHDWalletXPubYPubZPubUTXOsE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound, XpubNotSynced. Details: " + ", ".join(error_messages))
        else:
            return instance

...
class ListLatestMinedBlocksRIBSB(BaseModel):
    """
    Bitcoin
    """
    bits: StrictStr = Field(..., description="A sub-unit of BCH equal to 0.000001 BCH, or 100 Satoshi, and is the same as microbitcoincash (μBCH). Bits have two-decimal precision.")
    ...
    difficulty: Optional[StrictStr] = Field(None, description="Represents a mathematical value of how hard it is to find a valid hash for this block.")
    ...
    nonce: Optional[StrictInt] = Field(None, description="Represents a random value that can be adjusted to satisfy the proof of work")
    size: Optional[StrictInt] = Field(None, description="Represents the block size")
    ...
    __properties = ["bits", "chainwork", "difficulty", "merkleRoot", "nonce", "size", "strippedSize", "version", "versionHex", "weight"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ListLatestMinedBlocksRIBSB:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ListLatestMinedBlocksRIBSB:
        """Create an instance of ListLatestMinedBlocksRIBSB from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ListLatestMinedBlocksRIBSB.parse_obj(obj)

        _obj = ListLatestMinedBlocksRIBSB.parse_obj({
            "bits": obj.get("bits"),

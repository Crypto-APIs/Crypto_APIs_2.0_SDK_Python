...
DECODERAWTRANSACTIONHEXE400_ONE_OF_SCHEMAS = ["InvalidBlockchain", "InvalidNetwork", "InvalidPagination", "InvalidTransactionHex", "LimitGreaterThanAllowed", "UriNotFound"]

class DecodeRawTransactionHexE400(BaseModel):
    """
    ...
    oneof_schema_4_validator: Optional[InvalidBlockchain] = None
    # data type: InvalidNetwork
    oneof_schema_5_validator: Optional[InvalidNetwork] = None
    # data type: InvalidTransactionHex
    oneof_schema_6_validator: Optional[InvalidTransactionHex] = None
    actual_instance: Any
    one_of_schemas: List[str] = Field(DECODERAWTRANSACTIONHEXE400_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = DecodeRawTransactionHexE400.construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, InvalidNetwork):
            ...
            match += 1
        # validate data type: InvalidTransactionHex
        if not isinstance(v, InvalidTransactionHex):
            error_messages.append(f"Error! Input type `{type(v)}` is not `InvalidTransactionHex`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in DecodeRawTransactionHexE400 with oneOf schemas: InvalidBlockchain, InvalidNetwork, InvalidPagination, InvalidTransactionHex, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in DecodeRawTransactionHexE400 with oneOf schemas: InvalidBlockchain, InvalidNetwork, InvalidPagination, InvalidTransactionHex, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> DecodeRawTransactionHexE400:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> DecodeRawTransactionHexE400:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into InvalidTransactionHex
        try:
            instance.actual_instance = InvalidTransactionHex.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into DecodeRawTransactionHexE400 with oneOf schemas: InvalidBlockchain, InvalidNetwork, InvalidPagination, InvalidTransactionHex, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DecodeRawTransactionHexE400 with oneOf schemas: InvalidBlockchain, InvalidNetwork, InvalidPagination, InvalidTransactionHex, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        else:
            return instance

...
CREATEFUNGIBLETOKENTRANSACTIONREQUESTFROMADDRESSWITHOUTFEEPRIORITYE409_ONE_OF_SCHEMAS = ["InvalidData", "WalletAsAServiceAddressBalanceNotEnough", "WalletAsAServiceTokenNotSupported", "WalletAsAServiceWalletBalanceNotEnough"]

class CreateFungibleTokenTransactionRequestFromAddressWithoutFeePriorityE409(BaseModel):
    """
    ...
    actual_instance: Any
    one_of_schemas: List[str] = Field(CREATEFUNGIBLETOKENTRANSACTIONREQUESTFROMADDRESSWITHOUTFEEPRIORITYE409_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CreateFungibleTokenTransactionRequestFromAddressWithoutFeePriorityE409.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CreateFungibleTokenTransactionRequestFromAddressWithoutFeePriorityE409 with oneOf schemas: InvalidData, WalletAsAServiceAddressBalanceNotEnough, WalletAsAServiceTokenNotSupported, WalletAsAServiceWalletBalanceNotEnough. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CreateFungibleTokenTransactionRequestFromAddressWithoutFeePriorityE409 with oneOf schemas: InvalidData, WalletAsAServiceAddressBalanceNotEnough, WalletAsAServiceTokenNotSupported, WalletAsAServiceWalletBalanceNotEnough. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> CreateFungibleTokenTransactionRequestFromAddressWithoutFeePriorityE409:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> CreateFungibleTokenTransactionRequestFromAddressWithoutFeePriorityE409:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CreateFungibleTokenTransactionRequestFromAddressWithoutFeePriorityE409 with oneOf schemas: InvalidData, WalletAsAServiceAddressBalanceNotEnough, WalletAsAServiceTokenNotSupported, WalletAsAServiceWalletBalanceNotEnough. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match

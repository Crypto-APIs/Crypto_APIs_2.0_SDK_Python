...
LISTSYNCEDADDRESSESE400_ONE_OF_SCHEMAS = ["InvalidPagination", "InvalidXpub", "LimitGreaterThanAllowed", "UriNotFound"]

class ListSyncedAddressesE400(BaseModel):
    """
    ...
    actual_instance: Any
    one_of_schemas: List[str] = Field(LISTSYNCEDADDRESSESE400_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ListSyncedAddressesE400.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ListSyncedAddressesE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ListSyncedAddressesE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ListSyncedAddressesE400:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ListSyncedAddressesE400:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ListSyncedAddressesE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ListSyncedAddressesE400 with oneOf schemas: InvalidPagination, InvalidXpub, LimitGreaterThanAllowed, UriNotFound. Details: " + ", ".join(error_messages))
        else:
            return instance

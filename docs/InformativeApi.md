...
> ListSupportedTokensR list_supported_tokens(network)
...
with cryptoapis.ApiClient(configuration) as api_client:
    ...
    try:
        # List Supported Tokens
        api_response = api_instance.list_supported_tokens(network)
        pprint(api_response)
    except cryptoapis.ApiException as e:
    ...
    try:
        # List Supported Tokens
        api_response = api_instance.list_supported_tokens(network, context=context, limit=limit, offset=offset)
        pprint(api_response)
    except cryptoapis.ApiException as e:
        print("Exception when calling InformativeApi->list_supported_tokens: %s\n" % e)
```

"""
    CryptoAPIs

    Crypto APIs is a complex and innovative infrastructure layer that radically simplifies the development of any Blockchain and Crypto related applications. Organized around REST, Crypto APIs can assist both novice Bitcoin/Ethereum enthusiasts and crypto experts with the development of their blockchain applications. Crypto APIs provides unified endpoints and data, raw data, automatic tokens and coins forwardings, callback functionalities, and much more.  # noqa: E501

    The version of the OpenAPI document: 2021-03-20
    Contact: developers@cryptoapis.io
    Generated by: https://openapi-generator.tech
"""


import unittest

import cryptoapis
from cryptoapis.api.hd_wallets_api import HDWalletsApi  # noqa: E501


class TestHDWalletsApi(unittest.TestCase):
    """HDWalletsApi unit test stubs"""

    def setUp(self):
        self.api = HDWalletsApi()  # noqa: E501

    def tearDown(self):
        pass

    def test_derive_and_sync_new_change_addresses(self):
        """Test case for derive_and_sync_new_change_addresses

        Derive And Sync New Change Addresses  # noqa: E501
        """
        pass

    def test_derive_and_sync_new_receiving_addresses(self):
        """Test case for derive_and_sync_new_receiving_addresses

        Derive And Sync New Receiving Addresses  # noqa: E501
        """
        pass

    def test_get_hd_wallet__x_pub_y_pub_z_pub_assets(self):
        """Test case for get_hd_wallet__x_pub_y_pub_z_pub_assets

        Get HD Wallet (xPub, yPub, zPub) Assets  # noqa: E501
        """
        pass

    def test_get_hd_wallet__x_pub_y_pub_z_pub_details(self):
        """Test case for get_hd_wallet__x_pub_y_pub_z_pub_details

        Get HD Wallet (xPub, yPub, zPub) Details  # noqa: E501
        """
        pass

    def test_list_hd_wallet__x_pub_y_pub_z_pub_transactions(self):
        """Test case for list_hd_wallet__x_pub_y_pub_z_pub_transactions

        List HD Wallet (xPub, yPub, zPub) Transactions  # noqa: E501
        """
        pass

    def test_list_hd_wallet__x_pub_y_pub_z_pub_utxos(self):
        """Test case for list_hd_wallet__x_pub_y_pub_z_pub_utxos

        List HD Wallet (xPub, yPub, zPub) UTXOs  # noqa: E501
        """
        pass

    def test_list_synced_addresses(self):
        """Test case for list_synced_addresses

        List Synced Addresses  # noqa: E501
        """
        pass

    def test_prepare_a_utxo_based_transaction_from_hd_wallet__x_pub_y_pub_z_pub(self):
        """Test case for prepare_a_utxo_based_transaction_from_hd_wallet__x_pub_y_pub_z_pub

        Prepare A UTXO-Based Transaction From HD Wallet (xPub, yPub, zPub)  # noqa: E501
        """
        pass

    def test_prepare_an_account_based_transaction_from_hd_wallet__x_pub_y_pub_z_pub(self):
        """Test case for prepare_an_account_based_transaction_from_hd_wallet__x_pub_y_pub_z_pub

        Prepare An Account-Based Transaction From HD Wallet (xPub, yPub, zPub)  # noqa: E501
        """
        pass

    def test_sync_hd_wallet__x_pub_y_pub_z_pub(self):
        """Test case for sync_hd_wallet__x_pub_y_pub_z_pub

        Sync HD Wallet (xPub, yPub, zPub)  # noqa: E501
        """
        pass

    def test_sync_new_hd_wallet__x_pub_y_pub_z_pub(self):
        """Test case for sync_new_hd_wallet__x_pub_y_pub_z_pub

        Sync New HD Wallet (xPub, yPub, zPub)  # noqa: E501
        """
        pass


if __name__ == '__main__':
    unittest.main()
